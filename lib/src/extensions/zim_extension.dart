...
extension ZIMMessageExtend on ZIMMessage {
  ZIMKitMessage toKIT() {
    ...
    // Copy reactions from ZIMMessage to ZIMKitMessage (like iOS/Android)
    ret.reactions.value = reactions;

    // Copy repliedInfo from ZIMMessage to ZIMKitMessage (like iOS/Android)
    final zimRepliedInfo = repliedInfo;
    if (zimRepliedInfo != null) {
      ZIMKitLogger.logInfo(
          'toKIT: Found repliedInfo, messageID=${zimRepliedInfo.messageID}, senderUserID=${zimRepliedInfo.senderUserID}');
      // Get content summary based on message type
      String contentSummary = '';
      final messageInfo = zimRepliedInfo.messageInfo;
      switch (messageInfo.type) {
        case ZIMMessageType.text:
          final textInfo = messageInfo as ZIMTextMessageLiteInfo?;
          contentSummary = textInfo?.message ?? '';
          break;
        case ZIMMessageType.image:
          contentSummary = ZIMKitCore.instance.innerText.imageMessageText;
          break;
        case ZIMMessageType.video:
          contentSummary = ZIMKitCore.instance.innerText.videoMessageText;
          break;
        case ZIMMessageType.audio:
          contentSummary = ZIMKitCore.instance.innerText.audioMessageText;
          break;
        case ZIMMessageType.file:
          contentSummary = ZIMKitCore.instance.innerText.fileMessageText;
          break;
        case ZIMMessageType.combine:
          contentSummary = ZIMKitCore.instance.innerText.combineMessageText;
          break;
        default:
          contentSummary = '';
      }

      ret.replyInfo = ZIMKitReplyMessageInfo(
        messageID: zimRepliedInfo.messageID,
        senderUserID: zimRepliedInfo.senderUserID,
        senderUserName:
            zimRepliedInfo.senderUserID, // Will be updated by user info query
        messageType: messageInfo.type,
        contentSummary: contentSummary,
        originalMessage: null, // We don't have the full original message here
      );
    }
  ...
  String toStringValue() {
    switch (type) {
      ...
      case ZIMKitMessageType.revoke:
        return ZIMKitCore.instance.innerText.revokeMessageText;
      case ZIMMessageType.image:
...
extension ZIMUserFullInfoExtend on ZIMUserFullInfo {
  ...
  // Compatibility for accessing user ID
  String get id => baseInfo.userID;
}
