...
typedef ZIMFontSizeResolver = double Function(
    num fontSize, ZIMScreenUtil instance);

/// @nodoc
class ZIMScreenUtil {
  static const Size defaultSize = Size(750, 1334);
  static final ZIMScreenUtil _instance = ZIMScreenUtil._();
  ...
  ZIMFontSizeResolver? fontSizeResolver;

  ZIMScreenUtil._();

  factory ZIMScreenUtil() => _instance;
  ...
  static void configure({
    ...
    try {
      ...
      throw Exception(
          'You must either use ZIMScreenUtil.init or ScreenUtilInit first');
    }
  ...
  static Future<void> ensureScreenSizeAndInit(
    ...
    return ZIMScreenUtil.ensureScreenSize().then((_) {
      return configure(
        data: mediaQueryData,
        designSize: designSize,
        minTextAdapt: minTextAdapt,
  ...
  ZIMDeviceType deviceType() {
    ZIMDeviceType deviceType;
    switch (Platform.operatingSystem) {
      case 'android':
      case 'ios':
        deviceType = ZIMDeviceType.mobile;
        if ((orientation == Orientation.portrait && screenWidth < 600) ||
            (orientation == Orientation.landscape && screenHeight < 600)) {
          deviceType = ZIMDeviceType.mobile;
        } else {
          deviceType = ZIMDeviceType.tablet;
        }
        break;
      case 'linux':
        deviceType = ZIMDeviceType.linux;
        break;
      case 'macos':
        deviceType = ZIMDeviceType.mac;
        break;
      case 'windows':
        deviceType = ZIMDeviceType.windows;
        break;
      case 'fuchsia':
        deviceType = ZIMDeviceType.fuchsia;
        break;
      default:
        deviceType = ZIMDeviceType.web;
    }
    return deviceType;
  }
...
enum ZIMDeviceType { mobile, tablet, web, mac, windows, linux, fuchsia }
